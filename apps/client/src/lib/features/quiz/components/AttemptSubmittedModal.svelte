	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-[1px]">
		<div class="panel-surface w-full max-w-xl p-5 sm:p-6" role="dialog" aria-modal="true">
			<div class="mb-4 flex items-center justify-between gap-3">
				...
				<button class="btn-secondary" type="button" onclick={quizUiStore.closeAttemptSubmittedModal}>
					Cerrar
				</button>
			</div>
			...
			<div class="panel-muted grid gap-4 p-5 sm:p-6">
				...
				<div class="grid gap-3 sm:grid-cols-2">
					<div class="rounded-[6px] border border-zinc-200 bg-white p-4">
						<p class="m-0 flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-zinc-600">
							<UserRound size={13} />
							Estudiante
						</p>
					...
					<div class="rounded-[6px] border border-zinc-200 bg-white p-4">
						<p class="m-0 text-xs uppercase tracking-[0.16em] text-zinc-600">Enviado</p>
				...
				{#if summary.joinCode}
					<div class="rounded-[6px] border border-zinc-200 bg-white p-4">
						...
						<p class="mt-2 mb-0 font-mono text-lg tracking-[0.18em] text-black">{summary.joinCode}</p>
					</div>
				{/if}
			</div>
		</div>
