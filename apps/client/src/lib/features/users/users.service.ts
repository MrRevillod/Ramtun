...
class UsersService {
	listCollaboratorCandidates = async (query?: string): PromiseResult<ManagedUser[], AppError> =>
		request<ManagedUser[]>({
			method: 'GET',
	...
	listAssistantCandidates = async (query?: string): PromiseResult<ManagedUser[], AppError> =>
		request<ManagedUser[]>({
			method: 'GET',
			url: '/users',
			params: {
