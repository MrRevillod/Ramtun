...
<main class="app-shell">
	<header class="panel-surface flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
		<div>
			<p class="section-kicker">Área autenticada</p>
			<h1 class="mt-1 text-2xl leading-tight text-black sm:text-3xl">Cuestionarios y Tests de Certeza</h1>
		</div>

		<div class="flex flex-col gap-3 sm:flex-row sm:items-center">
			<div class="rounded-[4px] border border-zinc-200 bg-zinc-50 px-3 py-2 text-left sm:text-right">
				<p class="text-sm font-semibold text-zinc-800">{authStore.session?.user.name}</p>
